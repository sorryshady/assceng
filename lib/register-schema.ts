...
export const registerSchema = z.object({
  ...
  .transform((date) => {
    ...
    // Return an ISO string if parsing is successful.
    if (!isNaN(parsedDate.getTime())) {
      return parsedDate.toISOString();
    }

    // Returning an invalid date string if the date can't be parsed, which will be caught in next refine step.
    return "Invalid Date";
  })
  .refine((isoDate) => isoDate !== "Invalid Date", {
